...
// タグの配列を生成する
const createTags = (post: any) => {
  // 投稿者を除外する
  const pTags = post.tags.filter(
    (tag: any) => tag[0] == "p" && tag[1] != post.pubkey
  );
  return [...pTags, ["e", post.id, "", "root"], ["p", post.pubkey]];
};

// ハッシュを数値に変換する
const convertToNumber = (hash: any, range: any) => {
  const decimal = parseInt(hash, 16);
  return decimal % (range + 1);
};

// ハッシュを生成する
const generateHash = (value1: any, value2: any, value3: any) => {
...
const repliedPostIds = posts
  .filter((post: any) => post.pubkey == pk) // ボットか
  .map((post: any) => post.tags.find((tag: any) => tag[0] == "e")[1]);

// メンション
// prettier-ignore
const mentions = posts.filter(
  (post: any) => post.pubkey != pk &&                       // ボットではないか
  !repliedPostIds.includes(post.id) &&       // 返信済みではないか
  post.tags.every((tag: any) => tag[0] != "e") && // 返信に返信しない
  post.content.includes(`nostr:${npub}`)     // content にボットの npub を含むか
);

// 返信
const replies = mentions.map((post: any) =>
  finishEvent(
    {
      kind: 1,
